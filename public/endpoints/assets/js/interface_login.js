function submitQuery() {
  ...
  $.post('login/post', {username: username, password: password, mfatoken: mfatoken}, function(returnedData) {
    ...
    if (json !== null) {
      if (json['type'] == "error") {
        if (json['attributes']['error_code'] == "riskengine_error") {
          document.getElementById("re_error_ic").innerHTML = window.btoa(returnedData);
          $('#re_error').modal('show');
        } else if (json['attributes']['error_code'] == "riskengine_warning") {
          $('#re_warning').modal('show');
        } else if (json['attributes']['error_code'] == "user_mfarequired") {
      ...
      } else if (json['type'] == "response" && json['attributes']['response_code'] == "user_loginaccepted"){
        document.getElementById("token").value = window.btoa(json['payload']['token']);
